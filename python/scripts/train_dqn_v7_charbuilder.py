"""
Train v7 — random PG (character builder) + obs_v2 + self-play opzionale.

Setup:
  - Env: SelfPlayEnv con preset_a='random_pg', preset_b='random_pg'
  - Snapshot pool inizializzato col best v6 (se disponibile)
  - 1M step training
  - Eval su 3 condizioni:
    1. vs Utility AI sui 3 preset (matrice 3×3)
    2. vs Utility AI su 50 random PG vs random PG
    3. Self-play vs ultimo snapshot

Output: report dettagliato sulla performance per equipment combo.
"""
...
OUT_DIR = Path("/tmp/hex_tactics_dqn_v7_charbuilder")
...
class CharBuilderCallback(BaseCallback):
    def __init__(self, eval_freq=50_000, snapshot_freq=100_000, n_eval_ep=5, snapshot_pool=None):
        super().__init__()
...
def main():
    ...
    # Bootstrap: usa best v6 se esiste, else best v5
    v6_best = Path("/tmp/hex_tactics_dqn_v6_selfplay/best.zip")
    v5_best = Path("/tmp/hex_tactics_dqn_multi_v5/best.zip")
    if v6_best.exists():
        pool.add(str(v6_best))
        print(f"[bootstrap] v6 best al pool")
    elif v5_best.exists():
        pool.add(str(v5_best))
        print(f"[bootstrap] v5 best al pool")
    ...
    callback = CharBuilderCallback(
        ...
        n_eval_ep=5, snapshot_pool=pool,
    )
